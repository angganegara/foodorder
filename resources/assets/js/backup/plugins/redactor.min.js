	...
	Redactor.prototype = {
		...
		init: function()
		{
			...
			function afterBuild()
			{
				...
				if (this.opts.toolbar !== false)
				{
					this.$toolbar.find('a').prop('tabindex', '-1');
				}
			}
		...
		build: function(mobile, whendone)
		{
			if (mobile !== true)
			{
				...
				function initFrame()
				{
					this.$editor = this.$content.contents().find("body").prop('contenteditable', true).prop('dir', this.opts.direction);
				...
				else
				{
					...
					this.$el = $('<textarea name="' + this.$editor.prop('id') + '"></textarea>').css('height', this.height);
				}

				if (this.$editor)
				{
					this.$editor.addClass('redactor_editor').prop('contenteditable', true).prop('dir', this.opts.direction);
				}

				if (this.opts.tabindex !== false)
				{
					this.$content.prop('tabindex', this.opts.tabindex);
				}
			...
			else
			{
				if (this.$el.get(0).tagName !== 'TEXTAREA')
				{
					...
					var textarea = $('<textarea name="' + this.$editor.prop('id') + '"></textarea>').css('height', this.height).val(html);
					this.$el.hide();
		...
		destroy: function()
		{
			...
			else
			{
				...
				this.$box.remove();
				this.$editor.removeClass('redactor_editor').removeClass('redactor_editor_wym').prop('contenteditable', false).html(html).show();
			}
		...
		autoSave: function()
		{
			this.autosaveInterval = setInterval($.proxy(function()
			{
				$.ajax({
					...
					type: 'post',
					data: this.$el.prop('name') + '=' + escape(encodeURIComponent(this.getCode())),
		...
		buildColorPicker: function(dropdown, key)
		{
			...
			for (var i = 0; i < len; ++i)
			{
				...
				{
					_self.execCommand(mode, $(this).prop('rel'));

					if (mode === 'forecolor')
					{
						_self.$editor.find('font').replaceWith(function() {

							return $('<span style="color: ' + $(this).prop('color') + ';">' + $(this).html() + '</span>');
					...
					if (_self.browser('msie') && mode === 'BackColor')
					{
						_self.$editor.find('font').replaceWith(function() {
							...
							return $('<span style="' + $(this).prop('style') + '">' + $(this).html() + '</span>');
		...
		{
			$(this.getParentNode()).prop('color', '').css('color', '');
			this.syncCode();
		},
		...
		imageEdit: function(e)
		{
			...
			{
				$('#redactor_file_alt').val($el.prop('alt'));
				$('#redactor_image_edit_src').prop('href', $el.prop('src'));
				$('#redactor_form_image_align').val($el.css('float'));

				if ($(parent).get(0).tagName === 'A')
				{
					$('#redactor_file_link').val($(parent).prop('href'));
				}
		...
		imageSave: function(el)
		{
			...
			$(el).prop('alt', $('#redactor_file_alt').val());
		...
		{
			this._imageSet('<img src="' + $(e.target).prop('rel') + '" alt="' + $(e.target).prop('title') + '" />', true);
		},
			...
			var callback = $.proxy(function()
			{
				...
				if (this.browser('msie'))
				{
					...
					if (parent.nodeName === 'A')
					{
						...
						url = this.insert_link_node.prop('href');
						target = this.insert_link_node.prop('target');
					}
					else
					{
						if (this.oldIE())
		...
		_insertLink: function(a, text, link, target)
		{
			...
			if (text !== '')
			{
				if (this.insert_link_node)
				{
					...
					$(this.insert_link_node).prop('href', link);
					if (target !== '')
					{
						$(this.insert_link_node).prop('target', target);
					}
					else
					{
		...
		uploadInit: function(element, options)
		{
			...
			this.element_action = this.element.prop('action');
		...
		uploadForm : function(f, name)
		{
			...
			else
			{
				...
				f.prop('action', this.uploadOptions.url);
		...
		uploadLoaded : function()
		{
			...
			this.element.prop('action', this.element_action);
			this.element.prop('target', '');
