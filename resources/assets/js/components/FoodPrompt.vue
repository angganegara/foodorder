        ...
        <template v-if="isNotHP(product)">
          <table>
            <template v-for="price in product.prices">
              <tr v-if="! isSingleMeal(price) && price.type != 'sunday'" :key="price.id">
              ...
              <tr v-if="product.id == 3 && price.type == 'sunday' && form.mealOpt == 'weekly'" :key="price.id">
          ...
          <div v-if="form.mealOpt.length > 0">
            ...
            <template v-if="form.mealOpt == 'sunday'">
              <select v-model="form.deliveryDates" class="form-control-sm form-control block">
                <option value="">Select date</option>
                <option v-for="(date, i) in weeklyDates" :key="i" :value="date" :selected="weeklyDates.date1 == date ? 'true' : 'false'">{{ date }}</option>
              </select>
              <br>
            </template>
            <template v-else>
...
export default {
  ...
  watch: {
    ...
    'form.mealOpt': function () {
      ...
      if (this.form.mealOpt == 'fullday' || this.form.mealOpt == 'weekly' || this.form.mealOpt == 'singlemeal') {
        this.form.numberOfDays = this.form.mealOpt == 'weekly' ? 6 : 1
        this.$nextTick(() => {
          $(document).ready(($) => {
            var disabledDays = [0];
            var today = new Date()
      ...
      /*
      if (this.form.mealOpt == 'weeklyxxxx') {
